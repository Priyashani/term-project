...
$(document).ready(function () {
    ...
    function processBooks(books) {
        let fomattedString = searchString.trim().toLowerCase()
        books.forEach(element => {
            if(element.title.toLowerCase().includes(fomattedString)){
        ...
        if(allBooks.length < productFilter.pageSize){
            productFilter.totalPageCount = productFilter.pageSize
        } else{
    ...
    function loadErrorpage(){
        ...
        bookList.html('<div class="col mb-5"> No matching books found.</div>');
    }
